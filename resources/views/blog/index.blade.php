            ...
            <div class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
                ...
                @foreach($categories as $cat)
                    <a 
                        ...
                        class="px-4 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors {{ ($filters['category'] ?? '') == $cat->name ? 'bg-white text-dark-primary' : 'text-neutral-400 hover:text-white hover:bg-white/5' }}"
                    >
                        {{ $cat->name }}
                    </a>
                @endforeach
            ...
            @forelse($articles as $art)
                <article class="glass-card rounded-3xl overflow-hidden flex flex-col justify-between h-full border border-white/5 reveal-fade">
                    <div class="h-40 overflow-hidden relative">
                        ...
                        <span class="absolute top-3 right-3 px-2.5 py-0.5 rounded-full bg-dark-primary/80 text-[10px] font-bold text-brand-cyan uppercase tracking-wider">{{ $art['category'] }}</span>
                    </div>

                    <div class="p-4.5 flex-1 flex flex-col justify-between gap-4">
                        <div class="space-y-2">
