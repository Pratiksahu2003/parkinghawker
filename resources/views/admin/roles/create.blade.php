<x-admin-layout>
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-white tracking-tight">Create Role</h1>
        <p class="text-sm text-neutral-500 mt-1">Add a new user access role and define permissions.</p>
    </div>
    ...
    <div class="glass-card rounded-2xl p-6 max-w-2xl">
        <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required placeholder="e.g. parking owner" value="{{ old('name') }}" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-brand-cyan placeholder:text-neutral-600 transition-colors">
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    @forelse($permissions as $perm)
                        <div class="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 hover:bg-white/[0.04] transition-all cursor-pointer">
                            <input type="checkbox" name="permissions[]" id="perm_{{ $perm->id }}" value="{{ $perm->name }}" class="mt-0.5 h-4.5 w-4.5 rounded bg-white/5 border-white/10 text-brand-cyan focus:ring-brand-cyan focus:ring-opacity-25 focus:ring-2">
