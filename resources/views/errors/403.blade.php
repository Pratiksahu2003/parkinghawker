@extends('errors.layout')

@section('title', '403 — Forbidden')

@section('content')
<div class="w-full max-w-2xl mx-auto text-center space-y-8">

    <!-- Giant glowing code -->
    <div class="relative inline-block">
        <span class="text-[10rem] sm:text-[13rem] font-extrabold leading-none select-none"
              style="background: linear-gradient(135deg, #ef4444 0%, #ec4899 60%, #ef4444 100%);
                     -webkit-background-clip: text; -webkit-text-fill-color: transparent;
                     filter: drop-shadow(0 0 60px rgba(239,68,68,0.25));">
            403
        </span>
        <!-- Floating Shield/Lock -->
        <div class="absolute -top-4 -right-4 h-14 w-14 rounded-2xl bg-red-500/10 border border-red-500/30
                    flex items-center justify-center text-2xl shadow-lg shadow-red-500/10">
            🛡️
        </div>
    </div>

    <!-- Glass card -->
    <div style="background: rgba(30,41,59,0.45); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);
                border: 1px solid rgba(255,255,255,0.07); box-shadow: 0 8px 32px rgba(0,0,0,0.4);"
         class="rounded-3xl p-8 space-y-5">

        <div class="space-y-3">
            <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold uppercase tracking-wider">
                <span class="h-1.5 w-1.5 rounded-full bg-red-400 animate-pulse"></span>
                Access Denied
            </div>
            <h1 class="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
                No entry. This zone<br>is restricted
            </h1>
            <p class="text-neutral-400 text-sm leading-relaxed max-w-md mx-auto">
                You do not have the required permissions to view this resource. If you believe this is an error, please reach out to our support team.
            </p>
        </div>

        <!-- Actions -->
        <div class="flex flex-col sm:flex-row gap-3 pt-1">
            <a href="/"
               style="background: linear-gradient(135deg, #ef4444, #ec4899);"
               class="flex-1 text-center py-3 rounded-2xl text-white font-bold text-sm shadow-lg hover:opacity-90 transition-all">
                🏠 Go to Homepage
            </a>
            <a href="/contact"
               style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);"
               class="flex-1 text-center py-3 rounded-2xl text-neutral-300 font-semibold text-sm hover:text-white hover:bg-white/10 transition-all">
                📞 Contact Support
            </a>
        </div>
    </div>

    <p class="text-xs text-neutral-600">
        Error 403 · Forbidden Request
    </p>
</div>
@endsection
